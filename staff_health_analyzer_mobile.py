...
class StaffHealthAnalyzer:
    ...
    def __init__(
        self, data_path: str, api_key: Optional[str] = None, db_config: Optional[Dict[str, str]] = None
    ):
        """
    ...
    def _load_and_preprocess_data(self, data_path: str) -> pd.DataFrame:
        """Load and preprocess the dataset from CSV or database"""
        # Try loading from CSV first
        try:
            df = pd.read_csv(data_path)
            print(f"Successfully loaded data from CSV file: {data_path}")
        except Exception as e:
            print(f"Failed to load CSV data: {e}")
            # If CSV fails, try loading from database
            if self.db_config:
                try:
                    ...
                    query = """
                        ...
                        FROM health_data_table 
                    """
                    
                    # Fetch the data directly into a DataFrame
        ...
        # Handle BMI if missing
        if "bmi" not in df.columns:
            print("Warning: BMI data missing. Using estimated values.")
            df["bmi"] = DEFAULT_BMI_VALUE
...
if __name__ == "__main__":
    ...
    generated_report = analyzer.run_analysis(
        ...
        category="Gender",      # Age_range, Gender, BMI, Overall | Weekly, Monthly, Quarterly, Yearly
    ...
    all_trending_reports = analyzer.generate_all_reports("Trending")
